#pragma once
/**
 * port/include/JSystem/JGadget/linklist.h
 * PC port shadow of JGadget/linklist.h.
 *
 * The original file is nearly pure C++ template code with no GC-specific
 * intrinsics, so we pass it through with only the following changes:
 *   - Replace the original include guard with #pragma once
 *   - Replace GC-only assert macro with our port assert
 *   - Remove CodeWarrior __MWERKS__ block; keep the non-MW path
 */
...
struct TLinkListNode {
    TLinkListNode() { pNext_ = nullptr; pPrev_ = nullptr; }

    ~TLinkListNode() {
#if DEBUG
        JGADGET_ASSERTWARN(77, pNext_ == nullptr);
        JGADGET_ASSERTWARN(78, pPrev_ == nullptr);
#endif
    }

    TLinkListNode* getNext() const { return pNext_; }
    TLinkListNode* getPrev() const { return pPrev_; }
    void clear_() { pNext_ = nullptr; pPrev_ = nullptr; }
...
struct TNodeLinkList {
    struct iterator : public std::iterator<std::bidirectional_iterator_tag, TLinkListNode, s32,
                                           TLinkListNode*, TLinkListNode&> {
        iterator() { p_ = nullptr; }
        explicit iterator(TLinkListNode* pNode) { p_ = pNode; }
        iterator& operator=(const iterator& o) { p_ = o.p_; return *this; }
...
struct TLinkList : TNodeLinkList {
    ...
    struct iterator : TNodeLinkList::iterator {
        ...
        iterator& operator++() { TNodeLinkList::iterator::operator++(); return *this; }
        iterator& operator--() { TNodeLinkList::iterator::operator--(); return *this; }
        iterator operator++(int) { const iterator old(*this); ++*this; return old; }
        iterator operator--(int) { const iterator old(*this); --*this; return old; }
    ...
    struct const_iterator : TNodeLinkList::const_iterator {
        ...
        const_iterator& operator++() { TNodeLinkList::const_iterator::operator++(); return *this; }
        const_iterator& operator--() { TNodeLinkList::const_iterator::operator++(); return *this; }
        const_iterator operator++(int) { const const_iterator old(*this); ++*this; return old; }
        const_iterator operator--(int) { const const_iterator old(*this); --*this; return old; }
    ...
    T& front() { JUT_ASSERT(642, !empty()); return *begin(); }
...
struct TLinkList_factory : public TLinkList<T, I> {
    ...
    typename TLinkList<T, I>::iterator Erase_destroy(T* param_0) {
        typename TLinkList<T, I>::iterator it(this->Erase(param_0));
        Do_destroy(param_0);
        return it;
    }
};
...
template <typename T>
struct TEnumerator {
    TEnumerator(T _current, T _end) : current(_current), end(_end) {}
    bool isEnd()        const { return current != end; }
    operator bool()     const { return isEnd(); }
    T operator*() { T rv = current; ++current; return rv; }
    T current, end;
};

template <typename Iterator, typename T>
struct TEnumerator2 {
    TEnumerator2(Iterator _current, Iterator _end) : current(_current), end(_end) {}
    bool isEnd()    const { return current != end; }
    operator bool() const { return isEnd(); }
    T& operator*() { T& rv = *current; ++current; return rv; }
...
struct TContainerEnumerator
    : public TEnumerator2<typename T::iterator, typename T::value_type> {
    TContainerEnumerator(T& param_0)
        : TEnumerator2<typename T::iterator, typename T::value_type>(
